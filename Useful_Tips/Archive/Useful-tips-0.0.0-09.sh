...
printf "\n\t $LTCYN $BOLD Usage-$RESET $LTMAG
  sh $PROGNAME [$RESET$CYAN option | option-info$RESET $LTMAG ] $RESET
"
printf "\t $LTCYN $BOLD Options-$RESET $YELLOW
  ...
  Options Below have an 'INFO' option-
    [-info]\t Information\t  Add to the end of option the '-info' to get the more detailed\n\t\t\t\t  description and exit- EXAMPLE- $PROGNAME meta-info$RESET $CYAN
