...
function set_chroot
{
	PASSP="`echo 'Chrootdirectory /sftp/chroot/home/Zu' | sed -e 's/Z/\%/g'`"
printf "$LTCYN $BOLD
...
drwxr-xr-x 10 root sftponly 4096 Mar 16 12:15 sftpuser$RESET $YELLOW
  ...
  EXAMPLE /etc/ssh/sshd_config -$RESET $LTCYN
PermitRootLogin without-password
Subsystem   sftp    internal-sftp
AllowGroups LinuxAdmins crew colab sftponly root
Match group sftponly,!wheel,!users,!colab,!LinuxAdmins$RESET $YELLOW #The '!' means NOT this group$RESET $LTCYN
    ForceCommand internal-sftp
    AllowTcpForwarding no
    Chrootdirectory /sftp/chroot/home/<percent>u$RESET $YELLOW #Replace <percent> with SHIFT+5
\n $RESET
"
}
#
